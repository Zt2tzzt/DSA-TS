...
不平衡的节点称为 `root` / `grand`；在封装的 `AVLTreeNode` 中，通常用 `this` 表示。
...
1. 先找到**不平衡的节点（parent）**，根据平衡因子，判断该节点哪边不平衡（假设左边）；
2. 再找到 `pivot`，它的哪变造成了不平衡（假设左边），该情况就是左左（LL, Left, Left）

> 旋转不平衡节点时，关注的一定是：不平衡节点

### 1.左左（LL）

如果 `pivot` 是 `parent` 的左子节点，且 `pivot` 的左子节点，造成了树的不平衡。那么这个情况就是**左左（LL）**。

![左左-右旋转](NodeAssets/左左-右旋转.jpg)

如上图所示，左左情况，要向右旋转，这是固定的做法。

具体步骤如下：

Ⅰ、处理 `pivot` 节点。

- 找到 `pivot` 节点，`this.left`。
- 将 `pivot` 节点父节点，指向 `this.parent`

Ⅱ、处理 `pivot.right` 节点。

- 将 `this.right = pivot.right`
- 如果 `pivot.right` 有值，那么 `pivot.right.parent = this`

Ⅲ、处理 `this` 节点。

- 将 `pivot.right = this`；
- 将 `this.parent = pivot`；
- 将 `pivot` 挂载到原来 `this` 的父节点上（可能是挂载到左子节点、右子节点，根节点）
  - 判断原来的 `this`，是否有父节点；
  - 有，则将父节点的 `left` / `right` 指向 `pivot`；
  - 没有，则将 `this.root= pivot`。
...
右右情况，要左旋，与左左（LL）情况正好相反，实现步骤依此类推。
...
- 找到 `pivot` 节点，`this.right`。
- 将 `pivot` 节点父节点，指向 `this.parent`

Ⅱ、处理 `pivot.left` 节点。

- 将 `this.left = pivot.left`
- 如果 `pivot.left` 有值，那么 `pivot.left.parent = this`
...
- 左右（LR）情况，那么先对 `pivot` 进行左旋转（此时变为了左左），再对 `parent` 进行右旋转；
- 右左（RL）情况，那么先对 `pivot` 进行右旋转（此时变为了右右），再对 `parent` 进行左旋转；
...
先封装一个方法 `rebalance` 方法，让不平衡的节点，变得平衡。
...
在 `inser` 方法中，找到不平衡的节点。

重构 `BSTree` 类的 `inser` 方法，并新增一个受保护方法 `createNode`，用于创建节点。

重构 `BSTree` 类中的 `insertNode` 方法，每插入一个节点，设置它的 `parent` 属性。
