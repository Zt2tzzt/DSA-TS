...
> 平衡因子 = 左子树高度 - 右子树高度
>
> 平衡因子 > 1，表示左节点更高，parent 要右旋转；
>
> 平衡因子 < -1，表示右节点更高，parent 要左旋转；
>

旋转不平衡节点的步骤：

1. 先找到**不平衡的节点（parent）**，根据平衡因子，判断该节点哪边不平衡。
...
如果 `pivot` 是 `parent` 的左子节点，且 `pivot` 的左子节点，造成了树的不平衡；
...
具体步骤如下（下方的 `this` 代表 `parent`）：

Ⅰ、处理 `pivot` 节点。

1. 使用 `this.left`。找到 `pivot` 节点；
2. 将 `pivot` 节点父节点，指向 `this.parent`。

Ⅱ、处理 `pivot.right` 节点。

1. 将 `this.left = pivot.right`；
2. 如果 `pivot.right` 有值，那么 `pivot.right.parent = this`

Ⅲ、处理 `this` 节点的 `parent` 引用。

1. 将 `pivot.right = this`；
2. 将 `this.parent = pivot`；
3. 将 `pivot` 挂载到原来 `this` 的父节点上（可能挂载到左子节点、右子节点，或根节点）
   - 判断原来的 `this`，是否有父节点；
   - 有，则将父节点的 `left` / `right` 指向 `pivot`；
   - 没有，则将 `this.root = pivot`（这是 `ALVTree` 类中的操作）。
...
1. 使用 `this.right`，找到 `pivot` 节点。
2. 将 `pivot` 节点父节点，指向 `this.parent`。

Ⅱ、处理 `pivot.left` 节点。

1. 将 `this.right = pivot.left`
2. 如果 `pivot.left` 有值，那么 `pivot.left.parent = this`

Ⅲ、处理 `this` 节点的 `parent` 引用。

1. 将 `pivot.left = this`；
2. 将 `this.parent = pivot`；
3. 将 `pivot` 挂载到原来 `this` 的父节点上（可能是挂载到左子节点、右子节点，根节点）
   - 判断原来的 `this` 是否有父节点；
   - 有，则将父节点的 `left` / `right` 指向 `pivot`；
   - 没有，则将 `this.root = pivot`（这是 `ALVTree` 类中，`rebalance` 方法的操作）。
...
- 左右（LR）情况，要先对 `pivot.right` 进行左旋转（此时变为了左左），再对 `parent` 进行右旋转；
- 右左（RL）情况，要先对 `pivot.left` 进行右旋转（此时变为了右右），再对 `parent` 进行左旋转；
