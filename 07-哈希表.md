...
- 提高速度的一个办法，就是让哈希函数中，尽量少的有乘法、除法。因为它们的性能是比较低的。
...
- 步骤3：查看上一步的 bucket 是否为 null。为 null，表示之前在该位置没有放置过任何的内容，那么就新建一个桶（数组）[]；
...
- 因为，我们使用的是**链地址法**，允许 `loadFactor > 1`，所以这个哈希表，可以无限制的插入新数据。
...
- 但是这种情况下，**所有的数据项一定要同时进行修改，即在哈希化**（重新调用哈希函数，来获取到不同的位置）；
