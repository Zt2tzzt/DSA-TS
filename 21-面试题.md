...
如果字符串的反序，与原始字符串相同，则该字符串称为回文字符串。

考虑两种情况，以一个元素为中心；不是以一个元素为中心。
...
展开后的单链表，应该同样使用 TreeNode，其中 `right` 子指针，指向链表中下一个结点，而左子指针始终为 null 。

展开后的单链表，应该与二叉树先序遍历顺序相同。
