...
堆的本质，是一种特殊的树形数据结构，使用**完全二叉树**来实现：

- 堆可以进行很多分类，但是平时使用的基本都是**二叉堆**；
- 二叉堆又可以划分为**最大堆**和**最小堆**；

> 非完全二叉树，也可实现堆结构，但完全二叉树更方便使用换算公式。
...
> 如果要删除非子节点的节点，要进行“下滤”操作。

## 三、为什么需要堆结构？

对于每一种数据结构，都需要清楚为什么需要它，这是我们能够记住并且把握它的关键。
...
- 数组 / 链表：获取最大或最小值，时间复杂度是 O(n) 级别的；
  ...
  - 排序本身就会消耗性能；
- 哈希表：不需要考虑了；
- 树结构：获取最大或最小值，时间复杂度是 O(logn) 级别的；
  - 二叉搜索树，操作较为复杂，并且还要维护树的平衡时才是 O(log n) 级别；
  - AVL树，红黑树，也要消耗性能，维护树的平衡。
...
因为二叉堆可用完全二叉树表示；所以通常底层会使用数组来实现；

每个节点在数组中，对应的索引i（index）有如下的规律：

- 如果 i = 0 ，它是根节点；
- 在数组中，索引为 i 的节点，其**父节点**索引的计算公式：`floor( (i – 1) / 2 )`
- 在数组中，索引为 i 的节点，其**左子节点**索引的计算公式：`2i + 1`
- 在数组中，索引为 i 的节点，其**右子节点**索引的计算公式：`2i + 2`
...
demo-project\09-堆结构Heap\01-堆结构Heap（封装）.ts
