...
思路：将愿素组，看作是一个二叉堆结构，从第一个非叶子节点开始，进行下滤操作。

- 这种方式，称之为自下而上的下滤。也可以使用自上而下的上滤，但是效率较低，作为课下自行研究。
...
改变 `heapfy_up` 方法，挑出循环的比较条件；

改变 `heapfy_down` 方法中，最小索引的比较条件，挑出循环的比较条件。
