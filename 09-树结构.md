...
在二叉搜索树中，搜索最值是一件非常简单的事情，在下图中，甚至可用肉眼，直观的看出来了。
...
- 如果不满足，就判断该子节点，是父节点的左子节点，还是右子节点。再把相应的引用，设置为 `null` 即可。
...
`current` 只有左子节点，或者右子节点。

这个过程，考虑是否 `current` 就是根，

然后要改变父节点指向它的引用（可能是 `left` 或者 `right`），指向要删除节点子节点（可能是 `left` 或者 `right`）
...
如果我们要删除的节点，有两个子节点，甚至子节点还有子节点。

这种情况下，需要从它的子节点、孙子节点中，找到一个节点，来替换当前的节点。这个节点有如下特征：

- 最接近 current 的节点，即要么比 current 节点小一点点，要么比 current 节点大一点点。
...
第一种方法，会造成很大空间的浪费，特别是针对数据量较大的情况。
