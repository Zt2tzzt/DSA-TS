...
在简单排序算法诞生后的很长一段时间内，人们不断尝试发明各种各样的排序算法，但是当时的排序算法的时间复杂度都是`O(N²)`，看起来很难超越。
...
希尔排序的时间复杂度，取决于**步长序列**的选择，目前最优的步长序列，还没有被证明，因此希尔排序的时间复杂度，依然是一个开放的问题。

但是现在已经有很多更加优秀的排序算法：

- 比如：归并排序、快速排序...，所以从实际的应用角度来说，希尔排序已经使用的非常非常少了。
